...
setting = Setting()
setting.set_request_token("cQSbNY0g5Sj3YGsNW8GT9hTlZK2ZsxNn")
...
h_data = HistoricalData(kite_login, setting, "", logging)
if not h_data.prepare(True):
    logging.error("Cleaning old records failed")
    exit()

for token in tokens:
    h_data = HistoricalData(kite_login, setting, token, logging)
        
    if h_data.is_data_synced(setting.table_name_5m) or h_data.sync_five_min_data():
        ...
        if h_data.is_data_synced(setting.table_name_30m) or h_data.sync_thirty_min_data():
