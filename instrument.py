...
class Instrument:
    ...
    def __init__(self, token, is_index, logging):
        self.token = token
    ...
    def order_ids(self):
        return self.oredrs.keys()
        
    def load_historical_data(self, kite_login, setting):
        historical_data = HistoricalData(kite_login, setting, self.token, self.logging)
                ...
                if False and from_dt < to_dt:
                    try:
                        ...
                        if len(data_df) == candle_count:
                            ...
                            self.historical_data_5m.drop(columns=['previous_close', 'high_low', 'high_close', 'low_close'])
        ...
        if current_time > self.refresh_till_30m and current_time - self.refresh_till_30m < timedelta(minutes=30):
            ...
            if candle_30m["close"] > candle_30m["SMA-9-h"] + 3:
                ...
                if candle_5m["SMA-20"] > candle_5m["SMA-200"] + 2 and candle_5m["low"] <= candle_5m["SMA-20"] + 7:
                    ...
                    if candle_5m["SMA-20"] > pre_candle_5m["SMA-20"] + 0.5:
                ...
                candle_5m = self.historical_data_5m.iloc[-1]
                if candle_5m["SMA-20"] < candle_5m["SMA-200"] - 2 and candle_5m["high"] >= candle_5m["SMA-20"] - 7:
                    pre_candle_5m = self.historical_data_5m.iloc[-2]
                    if candle_5m["SMA-20"] < pre_candle_5m["SMA-20"] - 0.5:
                        candle_set = self.historical_data_5m.tail(3)
    ...
    def buy_ce_premium(self, kite_login, live_data, instrument_token):
        try:   
            token_data = live_data.get_current_data(int(self.token))
            ...
            if not ce_data_df.empty:
                ...
                stop_loss = ce_candle['low'] - 3
                target = ce_candle['high'] + 2 * parent_candle['ATR'] * 0.7
                ...
                if ce_candle['low'] < ce_price and  ce_price - stop_loss <= 20:
                    ...
                    order_id = order.buy_order(kite_login, ce_price, stop_loss, target, ce_candle)
        
                    if order_id is not None:
                        self.oredrs[order_id] = order
    ...
    def buy_pe_premium(self, kite_login, live_data, instrument_token):
        try:   
            ...
            if not pe_data_df.empty:
                ...
                stop_loss = pe_candle['low'] - 3
                target = pe_candle['high'] + 2 * parent_candle['ATR'] * 0.7
                ...
                if pe_candle['low'] < pe_price and pe_price - stop_loss <= 20:
                    ...
                    order_id = order.buy_order(kite_login, pe_price, stop_loss, target, pe_candle)
